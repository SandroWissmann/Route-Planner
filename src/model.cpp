...
void Model::LoadData(const std::vector<std::byte>& xml)
{
    ...
    for (const auto& way : doc.select_nodes("/osm/way")) {
        ...
        for (auto child : node.children()) {
            ...
            if (name == "nd") {
                ...
                if (auto it = node_id_to_num.find(ref);
                    it != end(node_id_to_num))
                    new_way.nodes.emplace_back(it->second);
            }
            else if (name == "tag") {
                auto category =
                    std::string_view{child.attribute("k").as_string()};
    ...
    for (const auto& relation : doc.select_nodes("/osm/relation")) {
        ...
        for (auto child : node.children()) {
            auto name = std::string_view{child.name()};
            if (name == "member") {
                if (std::string_view{child.attribute("type").as_string()} ==
                    "way") {
                    if (!way_id_to_num.count(
                            child.attribute("ref").as_string()))
                        continue;
                    auto way_num =
                        way_id_to_num[child.attribute("ref").as_string()];
        ...
        for (int i = 0; i < open_ways.size(); ++i)
            if (!used[i]) {
                ...
                if (way_head == tail || way_tail == tail) {
                    ...
                    if (way_head == tail)
                        nodes.insert(nodes.end(), way_nodes.begin(),
                                     way_nodes.end());
                    else
                        nodes.insert(nodes.end(), way_nodes.rbegin(),
                                     way_nodes.rend());
                    if (TrackRec(open_ways, ways, used, nodes))
                        return true;
                    nodes.resize(len);
