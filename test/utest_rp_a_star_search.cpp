...
TEST_F(RoutePlannerTest, TestAddNeighbors) {
    ...
    std::vector<float> start_neighbor_g_vals{
        0.10671431, 0.082997195, 0.051776856, 0.055291083};
    std::vector<float> start_neighbor_h_vals{
        1.1828455, 1.0998145, 1.0858033, 1.1831238};
...
TEST_F(RoutePlannerTest, TestConstructFinalPath) {
    ...
    std::vector<RouteModel::Node> path = route_planner.ConstructFinalPath(
        end_node);
...
TEST_F(RoutePlannerTest, TestAStarSearch) {
    ...
    // The start_node and end_node x, y values should be the same as in 
    //the path.
